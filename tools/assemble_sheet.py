...
def build_tube_cell(path: pathlib.Path, spec, factor: int, part: str):
    ...
    # Test for "magenta-ness" rather than a near-exact match. Where the accent
    # is seen through glass its colour shifts a long way from pure magenta, and
    # an exact test let those pixels through unconverted — which is how pink
    # streaks ended up on every junction tile. Nothing else in the palette has
    # both red and blue well above green, so this stays safe.
    r = img[..., 0].astype(np.int16)
    g = img[..., 1].astype(np.int16)
    b = img[..., 2].astype(np.int16)
    # No alpha condition: a barely-covered pixel still carries the marker
    # colour, and cleaning a transparent pixel costs nothing.
    flag = ((r - g) > 22) & ((b - g) > 22)
    cleaned = img.copy()
    cleaned[flag, 0:3] = np.array(LIGHT_BLUE, dtype=np.uint8)
    n = layout.CELL
...
def main() -> None:
    ...
    if args.sheet == "bridge":
        return assemble_bridge(args)
    if args.sheet == "tunnel":
        return assemble_tunnel(args)
    if args.sheet != "track":
...
BRIDGE_COLS = 8
BRIDGE_ROWS = 7          # header; span/pillars, ramps, starts; then winter


def bridge_icon() -> np.ndarray:
    """32x32 toolbar glyph: an elevated beam on two pylons."""
    img = np.full((layout.CELL, layout.CELL, 3), layout.KEY, dtype=np.uint8)
    img[10:15, 2:30] = (161, 168, 179)      # deck
    img[12:13, 2:30] = (90, 96, 106)        # slot line
    for x in (7, 22):
        img[15:28, x:x + 3] = (120, 118, 114)   # pylons
    return img


def assemble_bridge(args) -> None:
    """Pack a bridge sheet: RGBA (the tube classes carry real glass), with
    the tube cove marker-swapped to the reserved light exactly like the
    tube way sheets."""
    from PIL import ImageDraw

    cells_dir = pathlib.Path(args.cells)
    size = (BRIDGE_COLS * layout.CELL, BRIDGE_ROWS * layout.CELL)
    sheet = Image.new("RGBA", size, (0, 0, 0, 0))
    factor = args.supersample

    done, missing = 0, []
    for row in range(1, BRIDGE_ROWS):
        for col in range(BRIDGE_COLS):
            path = cells_dir / f"cell_{row}_{col}.png"
            if not path.exists():
                missing.append(f"{row}.{col}")
                continue
            ...
            out_a = np.zeros((n, n), dtype=np.uint8)
            out_rgb[keep] = np.round(np.clip(rgb[keep], 0, 1) * 255.0).astype(np.uint8)
            out_a[keep] = np.clip(np.round(alpha[keep] * 255.0),
                                  ALPHA_FLOOR, 255).astype(np.uint8)
            lit = cover >= 0.5
            ...
            out_a[lit] = 255
            out_rgb = avoid_special_colours(out_rgb, protect=lit)
            sheet.paste(Image.fromarray(np.dstack([out_rgb, out_a]), "RGBA"),
                        (col * layout.CELL, row * layout.CELL))
            done += 1

    sheet.paste(opaque(bridge_icon()), (6 * layout.CELL, 0))
    sheet.paste(opaque(tile_cursor()), (7 * layout.CELL, 0))

    draw = ImageDraw.Draw(sheet)
    draw.rectangle([0, 0, 6 * layout.CELL - 1, layout.CELL - 1],
                   fill=(255, 255, 255, 255))
    for i, line in enumerate([
            f"name: {pathlib.Path(args.out).stem.upper()}",
            "copyright: Aleksander Kwiatkowski - Artistic License 2.0",
            "",
            "generated by tools/blender/build_maglev_bridge.py",
            "r1 span bNS fNS bEW fEW pilS pilW; r2 ramps b/f nsew; "
            "r3 starts; r4-6 winter"]):
        draw.text((10, 12 + i * 14), line, fill=(0, 0, 0, 255))
...
TUNNEL_COLS = 5
TUNNEL_ROWS = 4          # summer front/back, winter front/back


def tunnel_icon() -> np.ndarray:
    """32x32 toolbar glyph: a portal face with a dark bore."""
    img = np.full((layout.CELL, layout.CELL, 3), layout.KEY, dtype=np.uint8)
    img[8:29, 3:29] = (150, 148, 144)               # headwall
    img[12:29, 8:24] = (14, 15, 18)                 # bore
    img[10:12, 6:26] = (120, 118, 114)              # lintel shadow line
    return img


def assemble_tunnel(args) -> None:
    """Pack a tunnel-portal sheet: RGBA, cove marker-swapped for the tube
    classes so the glow runs into the hill."""
    from PIL import ImageDraw

    cells_dir = pathlib.Path(args.cells)
    size = (TUNNEL_COLS * layout.CELL, TUNNEL_ROWS * layout.CELL)
    sheet = Image.new("RGBA", size, (0, 0, 0, 0))
    factor = args.supersample

    done, missing = 0, []
    for row in range(TUNNEL_ROWS):
        for col in range(4):
            path = cells_dir / f"cell_{row}_{col}.png"
            if not path.exists():
                missing.append(f"{row}.{col}")
    ...
    sheet.paste(opaque(tunnel_icon()), (4 * layout.CELL, 0))
    sheet.paste(opaque(tile_cursor()), (4 * layout.CELL, layout.CELL))

    draw = ImageDraw.Draw(sheet)
    draw.text((4 * layout.CELL + 4, 2 * layout.CELL + 8),
              "tunnel portals\nn w s e\nf/b, summer+winter",
              fill=(0, 0, 0, 255))
    scrub_sheet(sheet, protect_light=True).save(args.out)
    print(f"wrote {args.out}: {done} rendered cells"
